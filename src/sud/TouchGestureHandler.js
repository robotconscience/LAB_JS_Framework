SUD.require(SUD.src+"geom/Point.js");
 ...
 */
SUD.TouchGestureHandler = function() {
	"use strict";
	...
	function startMove(event) {
		...
		dispatch(SUD.TouchEvent.PRESS);
		
		var date = new Date();
		_startTime = date.getTime();
						
		_pressPoint = new SUD.geom.Point(event.clientX, event.clientY);
	}
	...
	function mouseMoveHandler(event) {
		...
		// save current point
		var currentPoint = new SUD.geom.Point(_touchX, _touchY);
		...
		if (distance > FLICK_MIN_MOVEMENT) {
			...
			if (!_isFlick) {
				...
				_flickStartPoint = new SUD.geom.Point(_touchX, _touchY);
			}
		...
		if (_isDrag) {
			...
			dispatch(SUD.TouchEvent.DRAG);
		}
		
		// update press point so we can have drag...drag...drag...flick
		_pressPoint = new SUD.geom.Point(currentPoint.x, currentPoint.y);
	...
	function mouseUpHandler(event) {
		...
		if (flickTimeChange <= FLICK_TIMEOUT && _isFlick) {
			...
			_flickEndPoint = new SUD.geom.Point(_touchX, _touchY);
	...
	function dispatch(type) {
		_self.dispatchEvent(new SUD.TouchEvent(type));
		...
		//_target.dispatchEvent(new SUD.TouchEvent(type, _self));
	}
...
SUD.TouchGestureHandler.prototype = SUD.inherit(SUD.EventDispatcher.prototype);
SUD.TouchGestureHandler.prototype.constructor = SUD.TouchGestureHandler;
...
// events
/** @constant */
SUD.TouchEvent.TAP = "touchtap";
/** @constant */
SUD.TouchEvent.FLICK = "touchflick";
/** @constant */
SUD.TouchEvent.DRAG = "touchdrag";
/** @constant */
SUD.TouchEvent.RELEASE = "touchrelease";
/** @constant */
SUD.TouchEvent.OUT = "touchout";
/** @constant */
SUD.TouchEvent.PRESS = "touchPress";

SUD.TouchEvent.prototype = SUD.inherit(SUD.Event.prototype);
SUD.TouchEvent.prototype.constructor = SUD.TouchEvent;