SUD.require(SUD.src+"TouchGestureHandler.js");

namespace("SUD.three.TouchHandler3D");
 ...
 */
SUD.three.TouchHandler3D = function(scene, camera) {
	"use strict";
	...
	function addGestureEventListener(type) {
		...
		var eventMap = {
						"touch3DDown": SUD.TouchEvent.PRESS,
						"touch3DUp": SUD.TouchEvent.RELEASE,
						"touch3DFlick": SUD.TouchEvent.FLICK,
						"touch3DDrag": SUD.TouchEvent.DRAG,
						"touch3DTap": SUD.TouchEvent.TAP,
						"touch3DOut": SUD.TouchEvent.OUT
						};
		
		if (!gestureHandler.hasEventListener(eventMap[type])) {
		
			switch (type) {
				case SUD.three.TouchEvent3D.DOWN:
					gestureHandler.addEventListener(SUD.TouchEvent.PRESS, onDownEvent);
					break;
				case SUD.three.TouchEvent3D.UP:
					gestureHandler.addEventListener(SUD.TouchEvent.RELEASE, onUpEvent);
					break;
				case SUD.three.TouchEvent3D.FLICK:
					gestureHandler.addEventListener(SUD.TouchEvent.FLICK, onTouchFlick);
					break;
				case SUD.three.TouchEvent3D.DRAG:
					gestureHandler.addEventListener(SUD.TouchEvent.DRAG, onTouchDrag);
					break;
				case SUD.three.TouchEvent3D.TAP:
					gestureHandler.addEventListener(SUD.TouchEvent.TAP, onTouchTap);
					break;
				case SUD.three.TouchEvent3D.OUT:
					gestureHandler.addEventListener(SUD.TouchEvent.PRESS, onOutEvent);
					break;
			}
		}
	}
				...
				if (hash[hitObject.id]) {
					...
					if (hitObject === hash[hitObject.id].object) {
						...
						if (events != undefined && events.indexOf(type) > -1) {
							...
							self.dispatchEvent(new SUD.three.TouchEvent3D(type), params);
						}
					} else {
	...
	function onTouchTap(evt) {
		handleEvent(SUD.three.TouchEvent3D.TAP, evt);
	}
	...
	function onTouchDrag(evt) {
		handleEvent(SUD.three.TouchEvent3D.DRAG, evt);
	}
	...
	function onTouchFlick(evt) {
		handleEvent(SUD.three.TouchEvent3D.FLICK, evt);
	}	
	...
	function onDownEvent(evt) {
		handleEvent(SUD.three.TouchEvent3D.DOWN, evt);
	}
	...
	function onUpEvent(evt) {	
		...
		handleEvent(SUD.three.TouchEvent3D.UP, evt);
	}
	...
	function onOutEvent(evt) {
		handleEvent(SUD.three.TouchEvent3D.OUT, evt);
	}	
...
SUD.three.TouchHandler3D.prototype = SUD.inherit(SUD.EventDispatcher.prototype);
SUD.three.TouchHandler3D.prototype.constructor = SUD.three.TouchHandler3D;


namespace("SUD.three.TouchEvent3D");
 ...
 */
SUD.three.TouchEvent3D = function(type) {
	"use strict";
	
	SUD.Event.call(this, type);
}

/** @constant */
SUD.three.TouchEvent3D.DOWN = "touch3DDown";
/** @constant */
SUD.three.TouchEvent3D.UP = "touch3DUp";
/** @constant */
SUD.three.TouchEvent3D.OUT = "touch3DOut";
/** @constant */
SUD.three.TouchEvent3D.TAP = "touch3DTap";
/** @constant */
SUD.three.TouchEvent3D.FLICK = "touch3DFlick";
/** @constant */
SUD.three.TouchEvent3D.DRAG = "touch3DDrag";

SUD.three.TouchEvent3D.prototype = SUD.inherit(SUD.Event.prototype);
SUD.three.TouchEvent3D.prototype.constructor = SUD.three.TouchEvent3D;
